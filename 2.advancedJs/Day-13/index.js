...
function display(arr){
    show.innerHTML=""
    arr.forEach(({image_url,name,price,id}) => {
        ...
        div.append(img,name1,price1,button)
        show.append(div)
    });
...
async function post(){
    let postData={
        ...
        id: document.querySelector("#Pid").value
    }
    let res= await fetch(`http://localhost:3000/products`,{
        method:'POST',
...
async function update(){
    ...
    let res= await fetch(`http://localhost:3000/products/${postData.id}`,{
        method:'PATCH',
        body:JSON.stringify(postData),
        headers:{
            'Content-Type':"application/json"
